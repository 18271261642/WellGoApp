...
/**
 * 作者:东芝(2018/8/23).
 * 功能:翻遍github上的未读消息红点开源库, 发现大部分
 * 不能适配很多布局,所以我写了一个能兼容全部的 !
 * 网上的写法是 继承TextView然后生成一个小红点drawable,设置到背景中去, 然后把目标view外层加一层FrameLayout,然后把小红点添加进去
 * 但这样做的问题来了, 小红点与目标View 会叠起来!, 挡住文字,  而且 他们提供的setOffsetX之类的没卵用,你如果想要偏移小红点让它不与下面的View重叠,那是不可能的
 * 所以我的写法是 为了更好的性能,直接继承View然后画小红点背景, 然后把目标view外层加一层LinearLayout 让小红点View放目标的右边,这样就不会重叠
 * 同时 我也提供原本的 重叠模式
 * 于是支持了 重叠目标模式 和 放目标右上角但不重叠的模式 两种模式 通过setOverlap 设置,  默认为false=不重叠
 */
        ...
        if (target.getParent() instanceof ViewGroup) {
            ...
            if (isOverlap) {//[小红点与目标View重叠]模式
                ...
                if (mIgnoreTargetPadding) {
                    layoutParams.setMarginEnd( target.getPaddingEnd() - w);
