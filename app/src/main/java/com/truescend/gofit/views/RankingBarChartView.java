 ...
 * 功能: 图表
 */
public class RankingBarChartView extends View {
    ...
    private RectF icon_rect = new RectF(); // 达标图片的 RECT位置
    ...
    private int icon_size = 40;//standards_size达标图片大小
    ...
    private int headIconIndexInData = -1;
    ...
    public RankingBarChartView(Context context) {
        super(context);
        init();
    }

    public RankingBarChartView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }

    public RankingBarChartView(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        init();
    }

    public RankingBarChartView(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        super(context, attrs, defStyleAttr);
        init();
    }
    ...
    private void init() {
        ...
        limitLinePaint = DIYViewUtil.createFillPaint(0xFF000000);
        limitLinePaint.setStrokeWidth(screenDensity * 2);
     ...
     * 头像位置
     */
    public void setHeadIcon(Bitmap headIcon, int indexInData) {
        this.headIcon = toRoundBitmap(headIcon);
        this.headIconIndexInData = indexInData;
        invalidate();
    }
    /**
     * 将图片转化为圆形头像
...
//            }
            int maxWidthLabel = 0;
            for (String label : mLimitLinesLabel) {
                Rect textRect = DIYViewUtil.getTextRect(label, labelPaint);
                if (maxWidthLabel < textRect.width()) {
                    maxWidthLabel = textRect.width();
                    maxTextString = label;
                }
...
//            float lx = -1, ly = -1;
        for (int i = 0; i < size; i++) {
            ...
            DIYViewUtil.setRectValue(barRect, cx - bar_width, cy, cx + bar_width, windowsRect.bottom + icon_size);


            DIYViewUtil.setRectValue(barPreviewRect, barRect.left, barRect.top, barRect.right, barRect.bottom - icon_size);
            //达标图片的内容若超过底部的可视范围(超过刻度线) 则不画 不然很难看
            if (!(icon_rect.bottom > barPreviewRect.bottom)) {
                icon_size = Math.round(4 * bar_width);
                //达标图片的 RECT位置
                DIYViewUtil.setRectValue(icon_rect, barRect.left - icon_size / 2 + barRect.width() / 2, barRect.top - icon_size, barRect.right + icon_size / 2 - barRect.width() / 2, barRect.top);
                int moveYOffset = (int) (icon_rect.height() * 0.3f);
                DIYViewUtil.paddingRect(icon_rect,0, -moveYOffset,0,moveYOffset);
                //实际 柱状可视范围
                //达标位置数据 需要和实际数据数量相同 不然不画
                if (headIcon != null && headIconIndexInData == i) {
                    DIYViewUtil.drawBitmapFill(canvas, headIcon, icon_rect, barPaint);
                }
            }
            canvas.save();
            //画 达标图片
...
//                canvas.restore();
        ...
        DIYViewUtil.setRectValue(windowsBottomScaleLineRect, windowsRect.left, windowsRect.bottom, windowsRect.right, windowsRect.bottom + mScaleLineRectHeight + padding);
        if (label != null) {
            //供子类 随意自定义
