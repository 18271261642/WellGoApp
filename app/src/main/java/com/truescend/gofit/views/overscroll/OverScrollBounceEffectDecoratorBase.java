package com.truescend.gofit.views.overscroll;
...
import com.truescend.gofit.views.overscroll.adapters.IOverScrollDecoratorAdapter;
import com.truescend.gofit.views.overscroll.adapters.RecyclerViewOverScrollDecorAdapter;

import static com.truescend.gofit.views.overscroll.IOverScrollState.STATE_BOUNCE_BACK;
import static com.truescend.gofit.views.overscroll.IOverScrollState.STATE_DRAG_END_SIDE;
import static com.truescend.gofit.views.overscroll.IOverScrollState.STATE_DRAG_START_SIDE;
import static com.truescend.gofit.views.overscroll.IOverScrollState.STATE_IDLE;
...
public abstract class OverScrollBounceEffectDecoratorBase implements IOverScrollDecor, View.OnTouchListener {
    ...
    protected static final int MAX_BOUNCE_BACK_DURATION_MS = 3000;
    ...
    protected class BounceBackState implements IDecoratorState, Animator.AnimatorListener, ValueAnimator.AnimatorUpdateListener {
        ...
        public Animator bounceBackAnim;
        ...
        protected ObjectAnimator createBounceBackAnimator(float startOffset) {
            ...
            ObjectAnimator bounceBackAnim = ObjectAnimator.ofFloat(view, mAnimAttributes.mProperty, 0);//东芝修改 去掉mStartAttr.mAbsOffset 改为0
            bounceBackAnim.setDuration(Math.max((int) bounceBackDuration, MIN_BOUNCE_BACK_DURATION_MS));
            bounceBackAnim.setInterpolator(new QuintOut());
    ...
    public boolean onTouch(View v, MotionEvent event) {
        switch (event.getAction()) {
            ...
            //东芝修改 添加以下代码 让回弹效果支持无限重刷, (原代码在布局回弹的过程中无法触摸View进行重刷操作)
            case MotionEvent.ACTION_DOWN:
                if(mCurrentState instanceof BounceBackState ){
                    Animator bounceBackAnim = ((BounceBackState) mCurrentState).bounceBackAnim;
                    if(bounceBackAnim!=null&&bounceBackAnim.isRunning()){
                        bounceBackAnim.cancel();
                        issueStateTransition(mIdleState);
                    }
                }
                break;
